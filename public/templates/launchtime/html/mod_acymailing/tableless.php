			...
			<div class="acymailing_form">
					<?php foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							echo '<label for="user_html_'.$formName.'">'.JText::_('RECEIVE').'</label>';
							...
							if($displayOutside) echo '<label for="user_'.$oneField.'_'.$formName.'">'.$fieldsClass->trans($extraFields[$oneField]->fieldname).'</label>';?>
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing::level(1)){ ?>
					...
					if(version_compare(JVERSION,'1.6.0','>=')){
						echo '<img class="captchaimagemodule" src="'.JRoute::_('index.php?option=com_acymailing&ctrl=captcha&acyformname='.$formName.'&val='.rand(0,10000)).'" alt="captcha" />';
					}else{
				...
				<?php }
				 ...
				 if($params->get('showterms',false)){
					...
					<span class="privacy-policy">
						...
						<input id="mailingdata_terms_<?php echo $formName; ?>" class="checkbox" type="checkbox" name="terms"/>
						By submitting this email, I understand the <?php echo $link; ?>
					</span>
					<!--</p>-->
					<?php } ?>
