<?php
/************************************************************************************
 mod_aidanews2 for Joomla 2.5 by Danilo A.
...
/* Preparation */
	...
	/* Load Pagination Javascript & CSS if needed */
	
	if ($params->get('aidapager')) {
		$document->addScript( 'modules/mod_aidanews2/lib/pagination/aidapager.js' );
		$document->addStyleSheet( 'modules/mod_aidanews2/css/pagination/' . $params->get('csspager') );
	}
	...
	/* Show Top Pagination if requested */
	
	if ($params->get('aidapager') && $params->get('pagerwhere') != 1) {
		echo '<div id="aidapagestop' . $module->id . '" class="paginationstyle">
			<a href="#" rel="aida_pager_previous">' . $params->get('pprevtext') . '</a> <span class="flatview"></span> <a href="#" rel="aida_pager_next">' . $params->get('pnexttext') . '</a>
			</div>';
	}
...
foreach ($list as $item) {
	...
	echo '<div class="aidanews2_art ' . $item->catcss . ' ' . $item->sticky . $item->today;
	if ($params->get('aidapager')) echo 'aidapage' . $module->id . ' hidepiece ';
	...
	if ($grid) { 
		if($params->get('grid_colw') == 'auto' || $params->get('grid_colw') == '') {
			//Make sure the cols don't add up to 100 (if there's an horizontal padding of even 1 pixel it won't display properly!)
			if (intval(100/$grid) * $grid == 100) $gw = intval(100/$grid)-1; else $gw = intval(100/$grid);
			echo '" style="width:' . $gw . '%">';
		}else{
			echo '" style="width:' . $params->get('grid_colw') . '">';
		}
	}else echo '" style="clear: both;">';
	
		/* Refresh patterns for every item */
		$patterns = array ('/\[title\]/', '/\[text\]/', '/\[empty\]/', '/\[readmore\]/', '/\[hits\]/', '/\[rating\]/', '/\[author\]/', '/\[image1\]/', '/\[image2\]/', '/\[image3\]/', '/\[date\]/', '/\[comments\]/', '/\[category\]/', '/\[br\]/', '/\[categories\]/', '/\[num\]/');
		$replace = array ($item->title, $item->text, '', $item->rm, $item->hits, $item->rating, $item->author, $item->img1, $item->img2, $item->img3, $item->date, $item->comments, $item->category, '<br/>', $item->categories, '<span class="aida_artnum_' . $counter . '">' . $counter . '</span>');
...
/* Show Bottom Pagination if requested */
	
	if ($params->get('aidapager') && $params->get('pagerwhere') != 0) {
		if ($params->get('langload')) {
			echo '<div id="aidapagesbottom' . $module->id . '" class="paginationstyle">
				<a href="#" rel="aida_pager_previous">' . JText::_('PAGER_PREV') . '</a> <span class="flatview"></span> <a href="#" rel="aida_pager_next">' . JText::_('PAGER_NEXT') . '</a>
				</div>';
		}else{
			echo '<div id="aidapagesbottom' . $module->id . '" class="paginationstyle">
				<a href="#" rel="aida_pager_previous">' . $params->get('pprevtext') . '</a> <span class="flatview"></span> <a href="#" rel="aida_pager_next">' . $params->get('pnexttext') . '</a>
...
/* Initialize AiDaPager Script */

	if ($params->get('aidapager')) {
		echo '<script type="text/javascript">
			var aidapages=new virtualpaginate({
				piececlass: "aidapage' . $module->id . '",' . "piececontainer: 'div', pieces_per_page: " . $params->get('pagernum') . ', 
				defaultpage: 0,	wraparound: false, persist: ' . (($params->get('pagerpersist')) ? 'true' : 'false') . '
			})
			';

			if ($params->get('pagerwhere') == 0)
				echo ' aidapages.buildpagination(["aidapagestop' . $module->id . '"])';
			elseif ($params->get('pagerwhere') == 1)
				echo ' aidapages.buildpagination(["aidapagesbottom' . $module->id . '"])';
			else
				echo ' aidapages.buildpagination(["aidapagestop' . $module->id . '", "aidapagesbottom' . $module->id . '"])';
